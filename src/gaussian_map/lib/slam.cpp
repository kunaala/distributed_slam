...
slam::slam(const Eigen::Vector3f vol_res, const Eigen::Vector3i vol_dim,std::string datafile, float ps_grid_res, float mu):
            agentT_(Eigen::Matrix4f::Identity()),
            dataseq_(datafile,0) {
                // {
    ...
    for(int i=0;i<180;i++) {
        angle_vec_<<thetai_ + i*ang_res_;
    }
}

Eigen::Matrix<float, 9, 3> slam::getPseudoPts(const Eigen::Vector3f p, float grid_side) {
    Eigen::Matrix<float, 9, 3> pts;
    pts<<p(0)-grid_side, p(1)-grid_side, p(3);
    pts<<p(0), p(1)-grid_side, p(3);
    pts<<p(0)+grid_side, p(1)-grid_side, p(3);
    pts<<p(0)-grid_side, p(1), p(3);
    pts<<p(0), p(1), p(3);
    pts<<p(0)+grid_side, p(1), p(3);
    pts<<p(0)-grid_side, p(1)+grid_side, p(3);
    pts<<p(0), p(1)+grid_side, p(3);
...
bool slam::mapNext() {
    ...
    if(dataPoint.first==0) { //ODOM datapoint
        ...
        newPose.block(0,0,2,2) = Rz*Ry*Rx;
        newPose.block(0,2,2,1) = tr;
