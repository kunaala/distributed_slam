...
int main(int argc, char *argv[] ) {
    ...
    while(ros::ok || !inpFile.eof()) {
        ...
        if(input[0]=="ODOM") {
            ...
            std::vector<double> pose{x_pose, y_pose, theta_pose, 0.0,0.0,0.0};

            ros_map.update_odom(pose);
            // vx = 0;
            // vy = 0;
            // vtheta = 0;
            ...
            ros_map.scan.header.stamp = scan_time;
            Mace.updatePose();
